...
def return_no_longer_existing_event_v2(
    old_events: list[CalendarDtoPickled], new_events: list[CalendarDtoPickled]
    ...
    """
    # Extract all titles from new_events
    new_titles = {_get_title_from_event_v2(event) for event in new_events}

    # Return old events whose titles are not in new_titles
    deleted_events = [
        event
        for event in old_events
        if _get_title_from_event_v2(event) not in new_titles
    ]
...
def _get_title_from_event_v2(event: CalendarDtoPickled) -> str:
    """
    Extracts the title from an event tuple.
    ...
    str
        The title of the event
    """
    if event is not None and isinstance(event.summary, str):
...
def return_new_list_of_series_which_actually_is_updated_v2(
    series_list: list[CalendarDtoPickled], old_series_list: list[CalendarDtoPickled]
) -> list[CalendarDtoPickled]:
    """
    Compares two lists of series and returns a list of series from the new list
    whose titles are not present in the old series list.
